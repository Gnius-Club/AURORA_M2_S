// A.U.R.O.R.A. Mission 2: Strategic Optimization
...
class AuroraGame {
    constructor() {
        this.gameData = {
            mission: {
                title: "A.U.R.O.R.A. Misión 2: Optimización de Secuencia y Estrategia",
        ...
        this.gameState = {
            ...
            roverDirection: 'norte', // norte, sur, este, oeste
            revealedTerrain: [],
            collectedSamples: [],
            currentSequence: [],
            stepsUsed: 0,
    ...
    setupEventListeners() {
        ...
        document.getElementById('execute-btn').addEventListener('click', () => {
            if (!this.gameState.isExecuting && this.gameState.currentSequence.length > 0) {
    ...
    setupDragAndDrop() {
        ...
        // Prevent default drag behavior on the drop zone
        ...
        dropZone.addEventListener('drop', (e) => {
            ...
            if (this.draggedCommand) {
                this.addCommandToSequence(this.draggedCommand);
                this.draggedCommand = null;
            }
        });

        // Set up event delegation for command blocks
        commandBlocks.addEventListener('dragstart', (e) => {
            if (e.target.classList.contains('command-block')) {
                ...
                // Store command data
                this.draggedCommand = {
                    commandId: e.target.dataset.commandId,
                    name: e.target.textContent,
                    hasParam: e.target.dataset.hasParam === 'true',
                    paramType: e.target.dataset.paramType
                };
                
                // Set drag data as fallback
                e.dataTransfer.setData('text/plain', JSON.stringify(this.draggedCommand));
                e.dataTransfer.effectAllowed = 'copy';
            }
        });

        commandBlocks.addEventListener('dragend', (e) => {
            if (e.target.classList.contains('command-block')) {
                e.target.classList.remove('dragging');
            }
        });

        // Also handle mobile touch events as fallback
        commandBlocks.addEventListener('click', (e) => {
            if (e.target.classList.contains('command-block')) {
                const commandData = {
                    commandId: e.target.dataset.commandId,
                    name: e.target.textContent,
                ...
                this.addCommandToSequence(commandData);
            }
        });
    }
    ...
    addCommandToSequence(commandData) {
        const sequenceItem = {
            ...
            hasParam: commandData.hasParam,
            paramType: commandData.paramType,
            paramValue: commandData.hasParam ? '' : null
        };
    ...
    updateSequenceDisplay() {
        ...
        // Hide/show drop message
        if (this.gameState.currentSequence.length > 0) {
            dropMessage.style.display = 'none';
        } else {
            dropMessage.style.display = 'block';
        }

        // Remove existing sequence items
        const existingItems = dropZone.querySelectorAll('.sequence-item');
        ...
        this.gameState.currentSequence.forEach((item, index) => {
            ...
            // Add event listener for parameter input
            if (item.hasParam) {
                const input = element.querySelector('.param-input');
                input.addEventListener('input', (e) => {
                    const sequenceItem = this.gameState.currentSequence.find(seq => seq.id === item.id);
                    if (sequenceItem) {
                        sequenceItem.paramValue = e.target.value;
                    }
                });
            }
            ...
            removeBtn.addEventListener('click', () => {
                this.removeSequenceItem(item.id);
            });
        });
    }

    removeSequenceItem(itemId) {
        this.gameState.currentSequence = this.gameState.currentSequence.filter(item => item.id !== itemId);
        this.updateSequenceDisplay();
        this.updateStepsCounter();
    }
    ...
    updateStepsCounter() {
        const stepsElement = document.getElementById('steps-used');
        stepsElement.textContent = this.gameState.currentSequence.length;
    }

    async executeSequence() {
        if (this.gameState.currentSequence.length === 0) {
            this.showFeedback('No hay comandos en la secuencia', 'error');
            return;
        }
        ...
        // Reset rover position and state for execution
        ...
        for (let i = 0; i < this.gameState.currentSequence.length && success; i++) {
            ...
            try {
                ...
                await this.delay(800); // Animation delay
            } catch (error) {
                success = false;
        ...
        if (success) {
            ...
            if (this.allObjectivesComplete()) {
                ...
                this.showFeedback('Secuencia ejecutada, pero faltan objetivos por completar', 'warning');
            }
        } else {
    ...
    async executeCommand(command) {
        switch (command.commandId) {
            case 'avanzar':
                await this.executeAvanzar(command.paramValue);
                break;
            case 'girar':
                await this.executeGirar(command.paramValue);
                break;
            case 'mapear':
                await this.executeMapear();
                break;
            case 'recoger':
                await this.executeRecoger(command.paramValue);
                break;
            case 'analizar':
                await this.executeAnalizar();
                break;
            case 'enviar':
                await this.executeEnviar();
                break;
            default:
                throw new Error(`Comando desconocido: ${command.commandId}`);
        }
    }
    ...
    async executeAvanzar(pasos) {
        ...
        if (isNaN(steps) || steps <= 0) {
            throw new Error('El número de pasos debe ser un número positivo');
        }
        ...
        for (let i = 0; i < steps; i++) {
            ...
            // Check bounds
            if (newPos.x < 0 || newPos.x >= this.gameData.mission.gridSize || 
                newPos.y < 0 || newPos.y >= this.gameData.mission.gridSize) {
                throw new Error('El rover intentó salir del área de misión');
            }

            // Check hidden terrain
            const isHidden = this.gameData.mission.hiddenTerrain.find(terrain => 
                terrain.x === newPos.x && terrain.y === newPos.y
            );
            const isRevealed = this.gameState.revealedTerrain.find(pos => 
                pos.x === newPos.x && pos.y === newPos.y
            );

            if (isHidden && !isRevealed) {
                throw new Error('El rover intentó entrar en terreno no mapeado');
            }
    ...
    async executeGirar(direccion) {
        ...
        if (!validDirections.includes(direccion.toLowerCase())) {
            throw new Error('Dirección inválida. Usa: norte, sur, este, oeste');
        }
    ...
    async executeMapear() {
        // Reveal all hidden terrain around rover (radius of 2)
        ...
        for (let dx = -2; dx <= 2; dx++) {
            for (let dy = -2; dy <= 2; dy++) {
                ...
                if (x >= 0 && x < this.gameData.mission.gridSize && 
                    y >= 0 && y < this.gameData.mission.gridSize) {
                    
                    const isHidden = this.gameData.mission.hiddenTerrain.find(terrain => 
                        terrain.x === x && terrain.y === y
                    );
                    
                    if (isHidden && !this.gameState.revealedTerrain.find(pos => pos.x === x && pos.y === y)) {
                        this.gameState.revealedTerrain.push({x, y});
                    }
                }
            }
        }
        ...
        this.createGrid(); // Refresh grid to show revealed terrain
        await this.delay(500);
    }

    async executeRecoger(tipo) {
        const roverPos = this.gameState.roverPosition;
        const objective = this.gameData.mission.objectives.find(obj => 
            obj.x === roverPos.x && obj.y === roverPos.y
        );

        if (!objective) {
            throw new Error('No hay nada que recoger en esta posición');
        }

        if (tipo.toLowerCase() !== objective.type) {
            throw new Error(`Se esperaba recoger "${objective.type}" pero se intentó recoger "${tipo}"`);
        }

        if (!this.gameState.collectedSamples.includes(tipo.toLowerCase())) {
            this.gameState.collectedSamples.push(tipo.toLowerCase());
            
            if (tipo.toLowerCase() === 'hielo') {
                this.gameState.objectives.ice = true;
            } else if (tipo.toLowerCase() === 'mineral') {
                this.gameState.objectives.mineral = true;
            }
        }
    ...
    async executeAnalizar() {
        if (this.gameState.collectedSamples.length === 0) {
    ...
    async executeEnviar() {
        ...
        if (roverPos.x === station.x && roverPos.y === station.y) {
            this.gameState.objectives.station = true;
        }
    ...
    getNextPosition() {
        ...
        const direction = this.gameState.roverDirection;
    ...
    checkObjectives() {
        ...
        const states = [
            this.gameState.objectives.mapped,
            this.gameState.objectives.ice,
            this.gameState.objectives.mineral,
            this.gameState.objectives.station
        ];
        ...
        objectives.forEach((id, index) => {
            ...
            if (states[index]) {
                ...
                element.querySelector('.objective-status').textContent = '●';
            }
        });
    }
    ...
    calculateScore() {
        ...
        let starCount = 0;
        if (steps <= this.gameData.scoring.threeStars) {
            starCount = 3;
        } else if (steps <= this.gameData.scoring.twoStars) {
            starCount = 2;
        } else if (steps <= this.gameData.scoring.oneStar) {
            starCount = 1;
        }
    ...
    allObjectivesComplete() {
        return this.gameState.objectives.mapped && 
               this.gameState.objectives.ice && 
               this.gameState.objectives.mineral && 
               this.gameState.objectives.station;
    }
    ...
    showSuccessMessage() {
        ...
        if (isOptimal) {
            ...
            this.showFeedback('¡Misión completada! Intenta optimizar tu solución para obtener 3 estrellas.', 'success');
        }
    }
    ...
    showFeedback(message, type = 'error') {
        ...
        feedbackElement.className = `feedback-message ${type === 'success' ? 'success' : ''}`;
    ...
    showTutorialStep() {
        const tutorialSteps = [
            {
                title: "Nueva Misión: Optimización Estratégica",
                description: "Especialista, A.U.R.O.R.A. funciona, pero de forma ineficiente. Tu misión: planifica la ruta más corta y eficiente posible.",
                highlight: "header"
            },
            {
        ...
        document.getElementById('tutorial-title').textContent = `Tutorial - Paso ${this.gameState.tutorialStep + 1}/7`;
        ...
        document.getElementById('tutorial-next').textContent = 
            this.gameState.tutorialStep === tutorialSteps.length - 1 ? 'Finalizar' : 'Siguiente →';

        // Highlight elements
        this.clearHighlights();
        if (step.highlight) {
            const element = document.getElementById(step.highlight) || document.querySelector(`.${step.highlight}`);
            if (element) {
